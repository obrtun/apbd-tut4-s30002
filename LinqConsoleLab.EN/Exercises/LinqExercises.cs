...
public sealed class LinqExercises
{
    ...
    {
        return [$"{UniversityData.Enrollments.Any(e => !e.IsActive)}"];
    }
    ...
    {
        return [$"{UniversityData.Lecturers.All(l => !string.IsNullOrWhiteSpace(l.Department))}"];
    }
    ...
    public IEnumerable<string> Task12_StudentCoursePairs()
    {
        return UniversityData.Enrollments
            .Join(
                ...
                e => e.StudentId,
                s => s.Id,
                (e, s) => new { e.CourseId, StudentName = $"{s.FirstName} {s.LastName}" })
            .Join(
                UniversityData.Courses,
                es => es.CourseId,
                c => c.Id,
                (es, c) => $"{es.StudentName} | {c.Title}");
    }
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        return UniversityData.Enrollments
            ...
            .GroupBy(x => x.Title)
            .Select(g => $"{g.Key} | {g.Average(x => x.Grade):0.00}");
    }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            ...
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName} | {g.Max(x => x.Grade):0.0}");
    }
    ...
    public IEnumerable<string> Challenge01_StudentsWithMoreThanOneActiveCourse()
    {
        return UniversityData.Students
            .Join(
                ...
                s => s.Id,
                e => e.StudentId,
                (s, e) => new { s.Id, s.FirstName, s.LastName })
            .GroupBy(x => new { x.Id, x.FirstName, x.LastName })
            .Where(g => g.Count() > 1)
    ...
    public IEnumerable<string> Challenge02_AprilCoursesWithoutFinalGrades()
    {
        return UniversityData.Courses
            ...
            .Where(c => UniversityData.Enrollments.Any(e => e.CourseId == c.Id))
            .Where(c => UniversityData.Enrollments
                .Where(e => e.CourseId == c.Id)
                .All(e => e.FinalGrade == null))
            .Select(c => c.Title);
    }
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        return UniversityData.Lecturers
            .Join(
                ...
                l => l.Id,
                c => c.LecturerId,
                (l, c) => new { Lecturer = $"{l.FirstName} {l.LastName}", CourseId = c.Id })
            .Join(
                UniversityData.Enrollments.Where(e => e.FinalGrade != null),
                lc => lc.CourseId,
                e => e.CourseId,
                (lc, e) => new { lc.Lecturer, Grade = e.FinalGrade!.Value })
            .GroupBy(x => x.Lecturer)
            .Select(g => $"{g.Key} | {g.Average(x => x.Grade):0.00}");
    }
